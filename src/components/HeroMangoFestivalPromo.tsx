    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="pointer-events-none relative z-30 w-full shrink-0 overflow-hidden px-4 pt-[max(0.75rem,env(safe-area-inset-top))] md:fixed md:inset-x-0 md:top-0 md:flex md:justify-end md:px-8 md:pt-7"
        >
          <motion.div
            ...
            className="hero-event-promo pointer-events-auto relative mx-auto w-full max-w-md md:mx-0 md:max-w-[22rem]"
          >
            {SPARKLES.map((s) => (
              <motion.span
                key={s.id}
            ...
            <div className="relative overflow-hidden rounded-2xl border-2 border-warm-gold/60 bg-midnight-crimson/94 px-4 py-4 shadow-[0_12px_40px_rgba(0,0,0,0.5)] backdrop-blur-md sm:px-5 sm:pr-12">
              ...
              <div className="relative text-center">
                <motion.p
                  className="text-[9px] font-semibold uppercase tracking-[0.18em] text-warm-gold sm:tracking-[0.22em]"
                ...
                <div className="mt-4 flex flex-col gap-2.5 sm:flex-row sm:justify-center sm:gap-2">
                  <motion.a
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="touch-target w-full rounded-full bg-brand-gold px-4 py-3 text-xs font-bold text-brand-crimson shadow-lg sm:w-auto sm:py-2.5"
                  >
