  ...
  return (
    <motion.article
      ...
      className={`group relative flex-shrink-0 cursor-pointer overflow-hidden rounded-2xl border-2 transition-colors ${
        isSelected
          ? "border-mango shadow-xl shadow-mango/20"
          : "border-forest/10 hover:border-forest/30"
      } ${isExpanded ? "w-full max-w-3xl" : "w-72 md:w-80"}`}
      onClick={handleSelect}
      data-program-id={program.id}
    >
      <div className={`relative ${isExpanded ? "md:flex" : ""}`}>
        <div
          className={`relative overflow-hidden ${
            isExpanded ? "md:w-1/2 h-56 md:h-auto md:min-h-[320px]" : "h-44"
          }`}
        >
          <Image
            src={program.image}
          ...
          {program.featured && (
            <span className="absolute right-3 top-3 rounded-full bg-mango px-3 py-1 text-xs font-bold text-forest">
              Featured 🥭
            </span>
          )}
        </div>

        <div className={`bg-linen p-5 ${isExpanded ? "md:w-1/2 md:p-6" : ""}`}>
          ...
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-lg font-bold text-mango">
              ₹{program.adultPrice.toLocaleString("en-IN")}
            </span>
            <span className="text-xs text-forest/50">Adults</span>
          ...
          {discount > 0 && isSelected && (
            <motion.p
              ...
              className="mt-2 rounded-lg bg-mango/15 px-3 py-1.5 text-xs font-semibold text-forest"
            >
            ...
            >
              ...
              <ul className="mt-3 space-y-1.5">
                {program.highlights.map((h) => (
                  ...
                  >
                    <span className="text-mango">✦</span> {h}
                  </li>
                ))}
              </ul>
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                {program.formUrl && (
                  <a
                    ...
                    className="rounded-full border border-forest/20 px-4 py-2 text-xs font-semibold text-forest transition-colors hover:border-mango hover:text-mango"
                  >
                    Register via Form
                  </a>
                )}
                <a
                  ...
                  className="rounded-full bg-mango px-5 py-2 text-xs font-bold text-forest shadow-md transition-all hover:scale-105"
                >
                  Book Now
                </a>
              </div>
      ...
      {isSelected && (
        <motion.div
          ...
          className="pointer-events-none absolute inset-0 rounded-2xl ring-2 ring-mango ring-offset-2 ring-offset-linen"
        />
      )}
    </motion.article>
  );
